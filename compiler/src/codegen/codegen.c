...
bool codegen_append(CodeGen* codegen, const char* code) {
    ...
    if (needed_size > (size_t)codegen->buffer_capacity) {
...
bool codegen_statement(CodeGen* codegen, ASTNode* stmt) {
    ...
    switch (stmt->type) {
        ...
        case AST_IF_STMT:
            return codegen_if_stmt(codegen, (ASTIfStmt*)stmt);
...
bool codegen_if_stmt(CodeGen* codegen, ASTIfStmt* if_stmt) {
    if (!codegen || !if_stmt) return false;

    codegen_indent(codegen);
    codegen_append(codegen, "if (");
    if (!codegen_expression(codegen, if_stmt->condition)) {
        return false;
    }
    codegen_append(codegen, ") ");

    if (!codegen_statement(codegen, if_stmt->then_branch)) {
        return false;
    }

    if (if_stmt->else_branch) {
        codegen_append(codegen, " else ");
        if (!codegen_statement(codegen, if_stmt->else_branch)) {
            return false;
        }
    }
