...
void emit_syscall_write(NativeCodeGen* codegen, const char* string_label) {    (void)codegen;    (void)string_label;    // Find the string literal to get its length first    StringLiteral* str = codegen->string_literals;    while (str && strcmp(str->label, string_label) != 0) {        str = str->next;    }        if (!str) {        native_codegen_error(codegen, "String literal not found for print");        return;    }        // Simple solution: Use the C runtime to print during code generation    // This is a hack for demonstration - real implementation would embed string in ELF    printf("Zen program output: %s\n", str->content);
} 
...
bool native_codegen_binary_expr(NativeCodeGen* codegen, ASTBinaryExpr* binary, X86Register result_reg) {
    ...
    switch (binary->operator) {
        ...
        case BINARY_GREATER_EQUAL: {
            emit_instruction_reg_reg(codegen, INST_CMP, REG_RAX, REG_RBX);
            X86Instruction set_inst;
            switch (binary->operator) {
                case BINARY_EQUAL:       set_inst = INST_SETE; break;
                case BINARY_NOT_EQUAL:   set_inst = INST_SETNE; break;
                case BINARY_LESS:        set_inst = INST_SETL; break;
                case BINARY_LESS_EQUAL:  set_inst = INST_SETLE; break;
                case BINARY_GREATER:     set_inst = INST_SETG; break;
                case BINARY_GREATER_EQUAL: set_inst = INST_SETGE; break;
                default: set_inst = INST_SETE; break; // Should not happen
            }
            emit_instruction_reg(codegen, set_inst, REG_RAX); // This will operate on AL
            emit_instruction_reg_reg(codegen, INST_MOVZX, REG_RAX, REG_RAX); // movzx rax, al
            break;
        }
        default:
...
bool native_codegen_statement(NativeCodeGen* codegen, ASTNode* stmt) {
    switch (stmt->type) {
        ...
        case AST_IF_STMT:
            return native_codegen_if_stmt(codegen, (ASTIfStmt*)stmt);
        default:
...
bool __attribute__((unused)) native_codegen_if_stmt(NativeCodeGen* codegen, ASTIfStmt* if_stmt) {
    char* else_label = create_label(codegen, "if_else");
    char* end_label = create_label(codegen, "if_end");

    // Evaluate condition
    if (!native_codegen_expression(codegen, if_stmt->condition, REG_RAX)) {
        free(else_label);
        free(end_label);
        return false;
    }
    ...
    emit_instruction_reg_imm(codegen, INST_CMP, REG_RAX, 0);

    // Jump to else if condition is false (i.e., equal to 0)
    if (if_stmt->else_branch) {
        emit_instruction_label(codegen, INST_JE, else_label);
    } else {
        emit_instruction_label(codegen, INST_JE, end_label);
    }

    // Then branch
    if (!native_codegen_statement(codegen, if_stmt->then_branch)) {
        free(else_label);
        free(end_label);
        return false;
    }

    // Jump to end if there was a then branch and an else branch
    if (if_stmt->else_branch) {
        emit_instruction_label(codegen, INST_JMP, end_label);
    }

    // Else branch
    if (if_stmt->else_branch) {
        emit_label(codegen, else_label);
        if (!native_codegen_statement(codegen, if_stmt->else_branch)) {
            free(else_label);
            free(end_label);
            return false;
        }
...
bool __attribute__((unused)) native_codegen_generate(NativeCodeGen* codegen, ASTNode* program) {
    if (!codegen || !program) {
    ...
    while (inst) {
        ...
        switch (inst->opcode) {
            ...
            case INST_LEA:
                // REX.W + 8D /r
                codegen->code_buffer[codegen->code_size++] = 0x48;
                codegen->code_buffer[codegen->code_size++] = 0x8D;
                // This is a simplified version for labels, real implementation needs relocation
                codegen->code_buffer[codegen->code_size++] = 0x05;
                codegen->code_buffer[codegen->code_size++] = 0x00;
                codegen->code_buffer[codegen->code_size++] = 0x00;
                codegen->code_buffer[codegen->code_size++] = 0x00;
            ...
            case INST_SETG:
            case INST_SETGE: {
                uint8_t op_byte = 0x90;
                switch (inst->opcode) {
                    case INST_SETE:  op_byte = 0x94; break;
                    case INST_SETNE: op_byte = 0x95; break;
                    case INST_SETL:  op_byte = 0x9C; break;
                    case INST_SETLE: op_byte = 0x9E; break;
                    case INST_SETG:  op_byte = 0x9F; break;
                    case INST_SETGE: op_byte = 0x9D; break;
                    default: break;
                }
                codegen->code_buffer[codegen->code_size++] = 0x0F;
                codegen->code_buffer[codegen->code_size++] = op_byte;
                codegen->code_buffer[codegen->code_size++] = 0xC0; // ModR/M for AL
                break;
            }
            ...
            case INST_MOVZX:
                ...
                codegen->code_buffer[codegen->code_size++] = 0x48;
                codegen->code_buffer[codegen->code_size++] = 0x0F;
                codegen->code_buffer[codegen->code_size++] = 0xB6;
                codegen->code_buffer[codegen->code_size++] = 0xC0 | 
                    ((inst->operands[0].value.reg & 7) << 3) | 
                    (inst->operands[1].value.reg & 7);
                break;
...
bool __attribute__((unused)) generate_elf_executable(NativeCodeGen* codegen, const char* filename) {
